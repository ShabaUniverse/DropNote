...
import PendingElement from "../components/PendingElement";
...
const Profile = () => {
  ...
  return (
    <div className=" pl-2 pt-10 md:pl-48">
      <h1 className=" pb-5 font-bold text-xl text-teal-500">Profile</h1>
      {/* post input */}
      <div className="post-area">
        <textarea
          placeholder="What you wanna share today...?"
          className=" bg-blue-100 border-none outline-none px-5 py-1 w-[340px] md:w-[500px] h-[150px] resize-none rounded-md"
          value={noteText}
      ...
      <div className="notes mt-10">
        ...
        {isPending === false ? (
          notes.length === 0 ? (
            <p className=" text-teal-500 text-lg font-bold text-center">You haven't posted yet</p>
          ) : (
            notes.map((item) => (
              <div
                className="note bg-blue-100 w-[340px] md:w-[500px] mb-10 px-5 py-2 rounded-md grid grid-cols-8"
                key={item.id}>
